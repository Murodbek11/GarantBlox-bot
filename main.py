...
TOKEN = "7377520849:AAE90iBIivs3iExzl1mm6QxwTt0MYDSG08I"  # <-- Новый токен
...
async def user_orders_status(message: Message):
    ...
    for o in user_orders:
        status = o['status']
        text_ = o['data'].get('description', 'Нет описания')
        lines.append(f"Заказ: {text_}\nСтатус: {status}")
    await message.answer("\n\n".join(lines))

@router.message(lambda m: m.text == "🧾 Моя реферальная ссылка")
async def send_ref_link(message: Message):
    user_id = message.from_user.id
    ref_link = f"https://t.me/YourBotUsername?start=ref_{user_id}"  # Заменить YourBotUsername
...
async def handle_text(message: Message):
    ...
    if text.startswith("📦") or text.startswith("📝") or text.startswith("✅") or text.startswith("❌"):
        return  # Игнорируем команды с этими эмодзи
...
async def approve_feedback(callback: CallbackQuery):
    ...
    username = feedback.get('user', 'Без_ника').lstrip('@')
